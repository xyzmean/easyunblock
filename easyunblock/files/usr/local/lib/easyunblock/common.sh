# easyunblock common helpers. Sourced (not exec'd) by every easyunblock-* script.
...
# crash-loops every tick. Don't add `set -u` to easyunblock-{failover,apply,status}.
...
SNI_MARK_VPN="0x1";       SNI_MARK_DIRECT="0x2";    SNI_NFQUEUE_NUM="220"
...
DNSMASQ_NFTSET_FILE="/tmp/dnsmasq.d/easyunblock-domains.conf"
...
# easyunblock ships only the WireGuard/AmneziaWG transport (the sing-box backend
# from the upstream fork was dropped for this standalone engine build). The
# transport-agnostic helpers below are kept as thin seams so callers (failover,
# doctor, firewall, ctl) don't carry transport conditionals — they all resolve
# to the wg path. A future transport can reintroduce dispatch here.
iface_is_tunnel() { iface_is_wg "$1"; }
device_is_tunnel() { device_is_wg "$1"; }

# Transport type of an endpoint. Always "wg" here (the only transport). Kept as
# a function so callers that already branch on it keep working unchanged.
ep_type() { echo wg; }

# ---- endpoint predicates / lifecycle (wg-only) -----------------------------
# Is there a live egress device for this endpoint? (wg = kernel link present).
ep_present() { iface_present "$1"; }
# L3 device for `ip route … dev` (wg = the iface itself).
ep_egress_dev() { echo "$1"; }
# Liveness age in seconds, smaller = healthier (wg = handshake age; 999999 down).
ep_liveness() { wg_handshake_age "$1"; }
# rx/tx cumulative bytes — echoes "rx tx" (wg = summed `wg show transfer`).
ep_transfer() {
    wgshow "$1" transfer \
        | awk '{rx+=$2; tx+=$3} END{printf "%d %d", rx+0, tx+0}'
}
# Bring an endpoint up. Callers that need to wait for liveness do so themselves
# (failover's wait_handshake).
ep_bringup() {
    ifup "$1" >/dev/null 2>&1 \
        || /etc/init.d/network reload >/dev/null 2>&1 || true
}
# MUST actually stop then start (full teardown+rebuild), not just start —
# easyunblock-failover's probe_candidate() calls ep_restart specifically for the
# "present but unhealthy, needs a full re-setup" rung and depends on this
# rebuilding the interface, mirroring wg's ifdown+ifup.
ep_restart() {
...
# `easyunblock-doctor --events`. Best-effort: a failed write never breaks failover.
EVENTS_FILE="/var/run/easyunblock-events"
EVENTS_MAX="200"
...
# zone, not just the tunnel. Used by easyunblock-firewall (refuse to touch it) and by
...
#
# Two intents, by EASYUNBLOCK_WAIT_LOCK:
...
single_run() {
    ...
    exec 9>"/tmp/easyunblock-$1.lock" || return 0
    if [ "${EASYUNBLOCK_WAIT_LOCK:-0}" = "1" ]; then
        # busybox flock has no -w; poll -n up to EASYUNBLOCK_LOCK_WAIT seconds.
        _sr_n=0
        until flock -n 9; do
            _sr_n=$((_sr_n + 1))
            [ "$_sr_n" -ge "${EASYUNBLOCK_LOCK_WAIT:-240}" ] \
...
# reachable from a read-only/diagnostic path. easyunblock-doctor is passive and does
