...
- ⚡ **Маршрутизация по SNI (ход конём)**: Интеграция с `zapret` (`nfqws`). Проверяется только 1-й пакет сессии в NFQUEUE, после чего ядро маркирует соединение (`connmark`) и качает гигабайты трафика напрямую или в VPN без участия CPU (нагрузка на проц 0%, решение проблем с Cloudflare и QUIC).
