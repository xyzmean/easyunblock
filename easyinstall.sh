#!/bin/sh
# easyunblock one-line easy installer with auto-configured WARP tunnel.
#
# Does everything install.sh does (easyunblock + AmneziaWG
...
#   - registers warp0 as the first endpoint and enables routing.
#
# After it finishes you have a working obfuscated WARP tunnel + routing,
...
if [ "$PKG_MANAGER" = "opkg" ]; then
    if ! opkg list-installed 2>/dev/null | grep -q "^nftables "; then
        ...
        opkg install nftables >/dev/null 2>&1 || warn "Не удалось установить nftables — easyunblock-firewall может не работать."
    fi
fi
...
# ──────────────────────────── 2. install easyunblock packages ──────────────
install_easyunblock() {
  if command -v easyunblock-ctl >/dev/null 2>&1 || [ -x /usr/local/sbin/easyunblock-ctl ]; then
    say "easyunblock уже установлен."
    return 0
  fi
  ...
  ls "$TMP/easyunblock-"*.$PKG_EXT >/dev/null 2>&1 || err "в релизе не хватает пакета easyunblock-*.$PKG_EXT"

  say "Устанавливаю easyunblock…"
  if [ "$PKG_MANAGER" = "apk" ]; then
...
# obfuscation knobs. Peer section naming follows easyunblock-ctl's wg-import format:
...
create_warp_iface() {
  ...
  # easyunblock owns routing (marks + table 200); never let netifd pull AllowedIPs
  # into the main table — matches the wg-import default.
  uci set "network.$WARP_IFACE.route_allowed_ips=0"

  # ── AWG obfuscation knobs (lowercase option names, as easyunblock-ctl emits) ──
...
register_in_splify() {
  ...
  if grep -q "option iface '$WARP_IFACE'" /etc/config/splify 2>/dev/null; then
    say "endpoint $WARP_IFACE уже зарегистрирован в конфигурации."
  else
    say "Регистрирую $WARP_IFACE как endpoint (приоритет 1)…"
...
EOF
  ...
  [ -x /etc/init.d/easyunblock ] && { /etc/init.d/easyunblock enabled 2>/dev/null || /etc/init.d/easyunblock enable; }
  /etc/init.d/easyunblock restart 2>/dev/null || true
}

# ──────────────────────────── 7. firewall zone ──────────────────────────────
# easyunblock owns routing (marks + table 200) but NOT the firewall — yet fw4 still
...
# Without this the tunnel "looks dead". easyunblock-firewall fix creates that zone
...
setup_firewall() {
  if [ ! -x /usr/local/sbin/easyunblock-firewall ]; then
    warn "easyunblock-firewall не найден — создайте зону для $WARP_IFACE вручную (masq + lan→зона)."
    return 0
  fi
  say "Создаю firewall-зону для $WARP_IFACE…"
  if /usr/local/sbin/easyunblock-firewall check "$WARP_IFACE" >/dev/null 2>&1; then
    say "Firewall-зона для $WARP_IFACE уже настроена."
  else
    /usr/local/sbin/easyunblock-firewall fix "$WARP_IFACE" \
      || warn "easyunblock-firewall fix не удался — проверьте зону для $WARP_IFACE в Сеть → Firewall."
  fi
}
...
sleep 5
say "Готово! Настроен обфусцированный WARP-туннель $WARP_IFACE + routing."
printf '  • Туннель:   %s (AmneziaWG + WARP, endpoint %s)\n' "$WARP_IFACE" "$WARP_EP"
printf '  • Адрес:     %s%s\n' "$WARP_V4" "${WARP_V6:+, $WARP_V6}"
printf '  • Управление: настройте /etc/config/splify и используйте easyunblock-ctl\n'
